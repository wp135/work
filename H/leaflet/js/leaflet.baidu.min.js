...
L.Projection.BaiduSphericalMercator = {
     ...
     Xr: function(h, b) {
         if (h && b) {
             var c = b[0] + b[1] * Math.abs(h.lng);
             var d = Math.abs(h.lat) / b[9];
 ...
 L.CRS.BEPSG3857 = L.extend({}, L.CRS.EPSG3857, {
     ...
     transformation: function() {
         var c = -18 - 8;
         var d = Math.pow(2, c);
         return new L.Transformation(d, .5, -d, .5);
     }()
 });
 ...
 L.TileLayer.BaiduLayer = L.TileLayer.extend({
     ...
     initialize: function(m, h) {
         var j = L.TileLayer.BaiduLayer.desc;
         m = m || "Normal.Map";
         var k = m.split(".");
         var n = k[0], l = k[1];
         var i = j[n][l];
     ...
     getTileUrl: function(j) {
         var i = Math.pow(2, j.z - 1), g = j.x - i, h = i - j.y - 1, f = L.point(g, h);
         f.z = j.z;
 ...
 L.TileLayer.BaiduLayer.desc = {
     ...
     customLayerDSN: {
         Map: "http://" + baseUrl + ":8080/sh/tilesDSN/newPics/{z}/{x}/{y}.jpg"
     },
     customLayerNormalSH: {
         //Map: "http://" + baseUrl + ":8080/sh/tilesBaidu/changgui/{z}/{x}/{y}.jpg"
        //    Map: "http://" + baseUrl + ":8080/sh/tilesBaidu/wuyelan/{z}/{x}/{y}.jpg"
           // Map: "http://" + baseUrl + ":8080/sh/tilesBaidu/lanse/{z}/{x}/{y}.jpg"
         //Map: BASEPATH.replace('LsmScreen','') + "sh/tilesBaidu/wuyelan/{z}/{x}/{y}.jpg"  //动态ip
         Map: "http://10.204.210.21:8080/tilesBaidu/{z}/{x}/{y}.jpg"  //动态ip
     },
     customLayerSatSH: {
         Map: "http://" + baseUrl + ":8080/sh/tilesBaidu/weixing/{z}/{x}/{y}.jpg",
         //Map: "http://" + baseUrl + ":19080/sh/tilesBaidu/satellite/{z}/{x}/{y}.jpg",
         Road: "http://" + baseUrl + ":8080/sh/tilesBaidu/luwang/{z}/{x}/{y}.jpg"
     },
 ...
 function wgs84togcj02(k, j) {
     var d = 3.141592653589793 * 3e3 / 180;
     var n = 3.141592653589793;
     var m = 6378245;
     var f = .006693421622965943;
 ...
 function transformlat(e, g) {
     var f = 3.141592653589793 * 3e3 / 180;
     var h = 3.141592653589793;
     var c = 6378245;
     var b = .006693421622965943;
     var d = -100 + 2 * e + 3 * g + .2 * g * g + .1 * e * g + .2 * Math.sqrt(Math.abs(e));
 ...
 function transformlng(e, g) {
     ...
     var c = 6378245;
     var b = .006693421622965943;
     var d = 300 + e + 2 * g + .1 * e * e + .1 * e * g + .1 * Math.sqrt(Math.abs(e));
     d += (20 * Math.sin(6 * e * h) + 20 * Math.sin(2 * e * h)) * 2 / 3;
 ...
 function gcj02tobd09(i, h) {
     var e = 3.141592653589793 * 3e3 / 180;
     var k = 3.141592653589793;
     var j = 6378245;
     var f = .006693421622965943;
     var g = Math.sqrt(i * i + h * h) + 2e-5 * Math.sin(h * e);
     var b = Math.atan2(h, i) + 3e-6 * Math.cos(i * e);
     var d = g * Math.cos(b) + .0065;
     var c = g * Math.sin(b) + .006;
     return [ d, c ];
 }

 function bd09togcj02(f, b) {
     var i = 3.141592653589793 * 3e3 / 180;
     var h = f - .0065;
     var g = b - .006;
     var e = Math.sqrt(h * h + g * g) - 2e-5 * Math.sin(g * i);
     var a = Math.atan2(g, h) - 3e-6 * Math.cos(h * i);
     var d = e * Math.cos(a);
     var c = e * Math.sin(a);
     return [ d, c ];
 }
